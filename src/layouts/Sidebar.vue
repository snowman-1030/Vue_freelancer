...
<template>
  <div
    class="flex flex-col h-screen relative overflow-x-hidden antialiased"
    :class="[
      isCollapsed ? 'w-20' : 'w-64',
      'transition-[width] duration-300 ease-in-out'
    ]"
    style="background-color: #12512A"
  >
    <!-- Logo Section -->
    <div class="h-16 flex items-center border-b border-white/10">
      <template v-if="!isCollapsed">
        <div class="flex-1 flex items-center px-6 py-1.5">
          <img
            src="@/assets/images/VUELEAF_logo_white.png"
            alt="VueLeaf Logo"
      ...
      <div class="flex items-center justify-center" :class="[isCollapsed ? 'w-full px-3' : 'px-4']">
        ...
        >
          <component
            :is="isCollapsed ? MenuIcon : ChevronLeft"
      ...
      <div v-for="section in menuSections" :key="section.title" class="mb-5">
        ...
        >
          ...
          <template v-if="!isCollapsed">
            ...
            <span 
              v-if="item.badge"
              class="ml-auto bg-white/10 text-white text-xs font-medium px-2 py-0.5 rounded-full"
            >
              {{ item.badge }}
            </span>
          </template>
    ...
    <div class="p-4 border-t border-white/10">
      <div 
        class="flex items-center gap-3 p-2 rounded-lg hover:bg-white/5 transition-all duration-200 cursor-pointer"
